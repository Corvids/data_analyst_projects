...
In the box plots, quality didn't vary much by wine type; the majority of the datapoints had a quality level of 5 or 6, although there white wines had an outlier that had a quality of 9.  This may be due to the fact that very few wines were rated a 3 or 9.
...
# doc: http://docs.ggplot2.org/0.9.3.1/stat_smooth.html
ggplot(data,aes(x=pH, y=alcohol, colour=type)) +
    geom_point(alpha=.3) +
    geom_smooth(alpha=.25, method = "loess", span = 1.5) +
    ggtitle("pH and Alcohol Scatterplot by Wine Type")

```

There isn't a clear pattern between alcohol and pH either for red or white wine.  The line for red wine slants upward after a pH of 3.5 but this may be due to outliers.
...
ggplot(data_selectQual, aes(x=pH, y=alcohol, colour = factor(quality))) +
  ...
  labs(title = 'pH vs alcohol scatterplot (factored by type and quality)',
      subtitle = 'Subsetted by quality levels of 5,6,7')

```

There appears to be weak correlation between pH and alcohol for both red and white wines at quality levels of 5, 6, and 7.
...
### pH and Alcohol scatterplot (factored by type and quality of 5, 6, or 7)
